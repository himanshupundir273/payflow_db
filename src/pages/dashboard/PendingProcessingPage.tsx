...
const PendingProcessingPage: React.FC = () => {
  ...
  const handleBulkProcess = async (ids: string[]) => {
    if (!user) return;
    const result = await bulkProcessPayments(ids);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Pending Processing</h1>
          {!isLoading && (
      ...
      <PaymentTable
        ...
        onAccountsQuery={user?.role === 'accounts' ? handleAccountsQuery : undefined}
        onBulkProcess={user?.role === 'accounts' ? handleBulkProcess : undefined}
...
export default PendingProcessingPage; 